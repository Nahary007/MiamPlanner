...
const Stock: React.FC = () => {
  ...
  const expiringCount = stockItems.filter(
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6 mb-8">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 md:p-6">
          <div className="flex items-center">
            <Calendar className="h-6 w-6 md:h-8 md:w-8 text-red-600 mr-3 flex-shrink-0" />
            <div className="min-w-0">
              <h3 className="text-sm md:text-lg font-semibold text-gray-900 truncate">
                Expirés
              </h3>
      ...
      <div className="mb-6">
        <div className="flex flex-wrap gap-2">
          ...
          >
            ...
            <span className="sm:hidden">Tous</span> ({stockItems.length})
          </button>
          <button
          ...
          >
            <span className="hidden sm:inline">Bientôt expirés</span>
            <span className="sm:hidden">Expire</span> ({expiringCount})
          </button>
          <button
          ...
          >
            <span className="hidden sm:inline">Expirés</span>
            <span className="sm:hidden">Expiré</span> ({expiredCount})
          </button>
        </div>
      </div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                    <div>
                      ...
                      <p className="font-medium text-gray-900 text-sm md:text-base">
                        {format(new Date(item.expirationDate), "dd MMM yyyy", {
                          locale: fr,
                        })}
                      </p>
                    </div>
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-medium ${expiration.color} text-center sm:text-left`}
                    >
                      {expiration.label}
                    </span>
                  </div>
